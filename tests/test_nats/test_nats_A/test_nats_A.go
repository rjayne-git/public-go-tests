...
* In this test scenario there is 1 message queue (subjectA) and 1 nats connection (connectionA).
* connectionA sends messages to subjectA. Those messageIds are added to a string set.
...
func main() {
	...
	defer cancel()
	// connectionA will send messages to subjectA and recv messages from subjectA.
	optsA := []nats.Option{}
