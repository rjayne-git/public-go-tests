...
const (
	appName         = "test-nats-stream-push-ubsub-ABC"
...
func main() {
	...
	*/
	...
	log.Print("Running jetstreamContextA.QueueSubscribe")
	if _, err = jetstreamContextA.QueueSubscribe(subjectA, groupA, func(msg *nats.Msg) {
		ackCountA++
	...
	}, nats.ManualAck(), nats.DeliverNew()); err != nil {
		log.Panicf("ERROR-A: nats.JetStreamContext.QueueSubscribe failed, err: %v", err)
	}
	// Start jetstreamContextB receive and print and resend routine.
	log.Print("Running jetstreamContextB.QueueSubscribe")
	if _, err = jetstreamContextB.QueueSubscribe(subjectB, groupB, func(msgIn *nats.Msg) {
		ackCountB++
		...
		log.Panicf("ERROR-B: nats.JetStreamContext.QueueSubscribe failed, err: %v", err)
	}
	// Start jetstreamContextC receive and print and resend routine.
	isSub := false
	subChan := make(chan struct{})
	unsubChan := make(chan struct{})
	endsubChan := make(chan struct{})
	go func() {
		recvFunc := func(msgIn *nats.Msg) {
			messageIndex := msgIn.Header.Get(messageIndexKey)
			messageId := msgIn.Header.Get(messageIdKey)
			message := string(msgIn.Data)
			if isSub {
				ackCountC++
				log.Printf("RECV-C: MessageId: %s, MessageIndex: %s, Body: %s, Ack", messageId, messageIndex, message)
				if err := msgIn.Ack(); err != nil {
				...
				nakCountC++
				log.Printf("RECV-C: MessageId: %s, MessageIndex: %s, Body: %s, Nak", messageId, messageIndex, message)
				if err := msgIn.Nak(); err != nil {
					log.Panicf("ERROR-C: nats.Msg.Ack failed, err: %v", err)
				}

			}
		}
		var subC *nats.Subscription
		var err error
		for true {
			select {
			case <-subChan:
				if !isSub {
					isSub = true
					log.Print("Running jetstreamContextC.QueueSubscribe")
					if subC, err = jetstreamContextC.QueueSubscribe(subjectC, groupC, recvFunc, nats.ManualAck(), nats.DeliverNew()); err != nil {
						log.Panicf("ERROR-C: nats.JetStreamContext.QueueSubscribe failed, err: %v", err)
					}
					subCountC++
				}
			case <-unsubChan:
				if isSub {
					isSub = false
					if subC != nil {
						log.Print("Running subC.Unsubscribe")
						if err = subC.Unsubscribe(); err != nil {
							log.Panicf("ERROR-C: nats.Subscription.Unsubscribe failed, err: %v", err)
						}
						unsubCountC++
					}
				}
	...
	go func() {
		...
		// Wait longer for this test case because in-flight messages lost during Unsubscribe must timeout and be resent.
		waitWithReason(time.Second, 60, "Giving time for all messages to arrive")
		log.Printf("ackCountA: %d", ackCountA)
		log.Printf("ackCountB: %d", ackCountB)
		log.Printf("ackCountC: %d", ackCountC)
		log.Printf("nakCountC: %d", nakCountC)
		log.Printf("subCountC: %d", subCountC)
		log.Printf("unsubCountC: %d", unsubCountC)
		messageSet.print()
